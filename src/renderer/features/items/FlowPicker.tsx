 ...
 * [OUTPUT]: 复选框左侧的流程色点：流程根点开 4×2 命名色板改色或不设流程；有上级时只读显示继承的流程色。
...
function FlowDot({ colors }: { colors: number[] }) {
    ...
    <div className="menu flow-menu">
      ...
      <div className="flow-swatches" role="radiogroup" aria-label={messages.flowColor}>
        {relationColors.map((_, index) => {
          ...
          return <button key={index} type="button" role="radio" aria-checked={color === index} aria-label={name} title={name} className="flow-swatch" disabled={busy || taken} onClick={() => choose(index)}>
      ...
      {color !== null && <>
        ...
        <button type="button" className="menu-item" disabled={busy} onClick={() => choose(null)}><Icon name="close" size={14} />{messages.noFlowColor}</button>
      </>}
    </div>
  </Popover>
}
