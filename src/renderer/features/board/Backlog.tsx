 ...
 * [OUTPUT]: Read-only saved-text links/previews, selection and explicit current-period arrangement requests.
...
export function Backlog({ horizon, revision, submit, busy, close, select }: { horizon: ItemHorizon; revision: number; submit: (action: Action) => Promise<unknown>; busy: boolean; close: () => void; select: (id: string) => void }) {
  ...
  return <Modal title={messages.backlogTitle(horizonNames[horizon])} close={close} wide>
    ...
    {page.items.map(item => {
      ...
      return <div className={`backlog-row${hasLinks ? ' backlog-row-links' : ''}`} key={item.id}>
        <input type="checkbox" aria-label={messages.selectItem(item.title)} checked={selected.has(item.id)} onChange={event => setSelected(previous => { const next = new Map(previous); if (event.target.checked) next.set(item.id, item); else next.delete(item.id); return next })} />
        {hasLinks ? <div className="backlog-link-content"><LinkTitle text={item.title} onOpen={() => { close(); select(item.id) }} /><small>{messages.originalPlan} {item.placement.periodId?.split(':').at(-1)}</small><LinkPreviews text={item.title} /></div> : <button onClick={() => { close(); select(item.id) }}>{item.title}<small>{messages.originalPlan} {item.placement.periodId?.split(':').at(-1)}</small></button>}
      </div>
    })}
