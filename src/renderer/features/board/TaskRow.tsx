/**
 * [INPUT]: Item summary, stable flow colors, status indicators and actions.
 * [OUTPUT]: Memoized accessible task row and keyboard/pointer drag controls.
 * [POS]: One virtual board row; Board owns placement and detail loading owns description bodies.
...
export const TaskRow = memo(function TaskRow({ index, total, item, flows, today, rolloverFrom, selected, dimmed, disabled, select, submit }: {
  index: number; total: number; item: ItemSummary; flows: Flows; today: string; rolloverFrom: string | undefined; selected: boolean; dimmed: boolean; disabled: boolean
  ...
  return <article role="listitem" aria-posinset={index + 1} aria-setsize={total} id={`item-${item.id}`} ref={setNodeRef} className={`task-row ${isDragging ? 'dragging' : ''}`} data-item-id={item.id} data-highlighted={selected} data-dimmed={dimmed} data-done={done}
