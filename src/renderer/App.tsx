...
export function App() {
  ...
  return <div className="app-shell">
    ...
    {!snapshot ? <main className="setup-page" role="status">正在打开本地工作区…</main> : !snapshot.workspace.setupConfirmedAt ? <Setup submit={submit} busy={busy} /> : view === 'board' ? <Board snapshot={snapshot} submit={submit} busy={busy} select={select} newRequest={newRequest} highlighted={highlighted} /> : <ItemList key={view} view={view} revision={snapshot.workspace.revision} select={select} timezone={snapshot.workspace.calendar!.timezone} />}
