...
export function App() {
  ...
  return <div className="app-shell">
    <TopBar ready={ready} flows={flows} filter={filter} setFilter={setFilter} columns={columns} active={palette ? 'search' : settings ? 'settings' : null}
    ...
    {!snapshot ? <main className="setup-page" role="status">{messages.opening}</main> : !ready ? <Setup submit={async action => { const result = await submit(action); if (result) setOnboarding(true); return result }} busy={busy} />
      ...
      <button className="fab" aria-label={messages.newItem} title="⌘N" disabled={busy} onClick={compose}><Icon name="add" size={24} strokeWidth={1.8} /></button>
    </>}
    ...
    {palette && <CommandPalette close={() => setPalette(false)} select={select} undo={() => void undo()} canUndo={!busy && undoCount > 0} create={compose} openSettings={openSettings} />}
