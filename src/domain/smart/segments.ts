/**
 * [INPUT]: 用户原文（不做语义理解）。
 * [OUTPUT]: segmentSlots：换行/分号/列表标记为强边界、逗号句号为弱边界的无损候选槽位（带原文偏移）；plainLater 普通模式单条标题/说明。
...
function pieces(text: string, offset: number, boundary: RegExp): { text: string; start: number }[] {
...
export function segmentSlots(text: string): Slot[] | null {
  ...
  strong.forEach((segment, index) => {
    const weak = pieces(segment.text, segment.start, /[，,。]/g).map(piece => trimmed(piece)).filter(piece => piece !== null)
