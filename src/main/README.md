...
├── storage/       # SQLite、备份与文件适配器、worker 通道，见局部地图
└── workspace/     # 权威事务、业务命令、历史/顺延与整库服务，见局部地图
```

`index.ts` 装配窗口与 StorageClient；`storage/worker.ts` 装配 SQLite 与 workspace 服务。业务规则由 `workspace → domain / shared/contracts` 消费，持久化通过 storage 落地。storage 的底层适配器不依赖 workspace；worker 是跨层组合入口。
