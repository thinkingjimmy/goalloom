...
const preview = (drafts: { id: string; source: string; horizon?: 'day' | 'later' | 'future'; certain?: boolean; due?: string | null }[], relations: SmartPreview['relations'] = []): SmartPreview => ({
...
it('计划负载带预览版本的既有上级与本批上级；本地复核颜色与根约束', () => {
  ...
  expect(draftProblem(drafts.map((row, i) => i === 0 ? { ...row, parents: [{ kind: 'draft', draftId: drafts[1]!.id }] } : row), parents, [])).toContain('循环')
})
