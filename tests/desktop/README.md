...
从仓库根调用 `npm run test:electron`、`test:ui`、`test:history`、`test:recovery`、`test:performance`。窗口脚本接受可执行文件路径，例如 `npm run test:ui -- release/mac-arm64/Goalloom.app/Contents/MacOS/Goalloom`。`dialogs.mjs` 需要人工操作，不纳入自动检查。
