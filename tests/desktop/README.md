...
├── workspace.mjs        # 看板业务、协议/CSP/IPC、主题与窄窗口
├── history.mjs          # 历史/往期/hold、恢复后的缓存与会话隔离
├── recovery.mjs         # 保护备份/维护/重置/SQLite 恢复与重启暂停
├── composer.mjs         # 可跳过 Onboarding、全局 composer 普通 Later/会话草稿、列头＋Enter/Space 与 Tab 步数、拆解
...
从仓库根调用 `pnpm test:electron`、`test:ui`、`test:history`、`test:recovery`、`test:composer`、`test:performance`。`composer.mjs` 把 Tab 步数写入 `output/tests/composer.json`。窗口脚本接受可执行文件路径，例如 `pnpm test:ui release/mac-arm64/Goalloom.app/Contents/MacOS/Goalloom`。`dialogs.mjs` 需要人工操作，不纳入自动检查。
