...
try {
  ...
  await page.getByRole('button', { name: '流程颜色：蓝', exact: true }).click()
  ...
  await page.getByRole('button', { name: '流程颜色：青', exact: true }).waitFor()
  ...
  await page.getByRole('button', { name: /^保存/ }).click()
  await page.waitForFunction(async () => { const item = (await window.goalloom.listItems({ type: 'list', view: 'search', query: '撤销内容保留', offset: 0, limit: 50 })).items[0]; return item && (await window.goalloom.getItem(item.id)).item.description === '撤销创建后必须保留的文本' })
  ...
  const settingsDialog = page.getByRole('dialog', { name: '设置与数据' }), detail = page.getByRole('dialog', { name: '当前条目' })
