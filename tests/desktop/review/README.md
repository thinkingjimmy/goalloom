...
- `renderer.mjs`: current-source Chromium with delayed IPC; input revisions, late previews, pagination and shortcuts.
- `lifecycle.mjs`: real Electron reload, close/reopen and crash maintenance release.
- `virtual.mjs`: real Electron long columns; bounded DOM, keyboard/pointer drag, logical focus, folds and refresh counts.
...
`pnpm test:review` is included in `pnpm verify`. The large-backup, scaling and performance suites are separate explicit commands. Artifacts go to ignored `output/tests/review-fixes/`, `output/tests/performance/` and `output/tests/packages/`.
