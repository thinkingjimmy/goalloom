...
it('重置成功原子回配置、保留主题/备份、拒绝旧代次，重复提交返回同一结果', async () => {
  create(); run({ type: 'preferences', theme: 'dark' }); run({ type: 'preferences', style: 'minimal' })
  ...
  expect(repo.store.workspace()).toMatchObject({ calendar: null, setupConfirmedAt: null, pausedAfterRestore: false, theme: 'dark', style: 'minimal' })
