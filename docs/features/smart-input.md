...
Enter 像输入法上屏一样执行当前主操作（Cmd/Ctrl+Enter 同义），Shift+Enter 换行；IME 组合输入不提交/不启动新云请求。列内原 Enter 创建不变。手动字段和手动结构优先，后续判断不能覆盖；原文大改后无法映射时保留手动内容并提示整理。候选结果与输入、手动修订、上下文均匹配时才可用。

关闭未保存内容可保留当前会话草稿或放弃；退出提醒未保存，整库替换清理旧草稿。保存失败不清输入、不关闭窗口。保存成功只清理已提交的输入修订；保存期间及随后的权威快照刷新期间新输入的文字继续保留。迟到判断只处理当前修订，不重新添加已手动移除的草稿；保留已选上级、待确认上级建议及当前候选的身份和版本。模型的选项分布不足以形成明确建议时，按负责人 2026-09-25 的决定：拿不准的执行列与上级默认采用进推荐（关联规则需要时一并移列），拿不准的截止与片段角色仍只作提示；确定性校验通过仍必须由用户确认（↵），每项都可在调整中撤回。

弹窗可在保存期间关闭；草稿会话与保存回执由当前工作区持有，关闭仅卸载弹窗并取消分析。成功回执即使在关闭后到达，也只清理已提交修订；重新打开后新增的文字保留，失败保留原稿。不能把已保存内容再次作为未保存草稿恢复。
...
日期范围的第二端即使省略月份（如「9月24日至26日」），也整体保留为待确认范围，不能用起点预填确定截止日。「本月月底」「这个月月底」及「月底」按完整表达解析。
...
**总数 `Q = 1 + 4S + D + 3 + R ≤ 64`（2026-09-25 起每槽位加一道执行时间推测题，见§14.4）。** 最大 S=8、D=8 时，核心44题，关系最多20题。8项×8个已有候选再加本批56个有向对，一共需156题，不能全问。即使把本批关系压成8道单父Choice，也仍要108题，且会改变多父语义；首版不采用这种替代。
...
**选项一致性与原值保留：** 总和误差预算不能用来容忍选错标签。`choice`在返回分布中的值应满足 `max(p) - p[choice] ≤ ε`（阈值不放宽）。2026-09-25 实测 OpenRouter 偶发近似并列却选了较小项（如 part 0.49、task 0.50）：不满足时该字段的分布记为不一致，只作未确认建议、不参与自动预填；不因单题拒绝整批响应，其他合法字段照常使用。保留供应商原分布及舍入来源，不原地重写。总和S大于0且分布已校验时，才以副本 `q_i = p_i / S` 计算下面的熵指标；不会据此声称恢复了舍入前的真实概率。全零等无可用概率信息的分布不归一化、不生成确定指标；原choice仍可作为未确认建议。[R11]
...
启动状态查询只加载轻量配置服务；题单/预览与供应商 SDK 在实际判断或连接测试时按需加载。取消从配置/凭据异步预检开始生效。renderer 关闭、重载、崩溃或整库替换释放整个智能输入会话：中止判断和连接测试、清空预览缓存，保留设备配置与加密凭据。
...
| V07 | S=8/D=8/已有根=8/本批56对 | 核心44＋关系≤20，累计≤64，未评估关系标明且可手动补 |
...
| V30 | 无confidence/缺分布，K=2/8/32，d=2/3及缺失/非法元数据，合法舍入与真坏分布 | 同分布＋同精度策略一致；32项两位合法误差按0.16+ε而非固定小值；三位缩至0.016+ε，超界/非法键值拒绝；choice最大值检查不放宽（不满足的单题待确认、不拒绝整批），全零/未知精度待确认，不补造概率 |
...
- **已知取舍：** 当前题单均在已知状态下定义，仅当 1+4S+D+3+R 超过 64 时，第一轮只问核心题，分组确定后发一次具名补充请求问任务之间及与候选的关系（累计仍 ≤64）；用户在预览中手选的上级不回传给 Jev 重复询问，直接以手动结构为准。
...
### 14.4 实测调优（2026-09-25，OpenRouter · typesafe/jev-1.13）

`pnpm eval:smart` 用真实 OpenRouter 跑 `scripts/eval/cases.ts` 的 25 条中文样例（固定参考日 2026-09-23 周三），产物为 `output/eval/smart-<时间>.{json,md}`（含逐题原始 choice/概率、拆分、字段与耗时），Key 只从 git 忽略的 `.env.local` 读取。首轮 14 条基线 11/14；修正后 25/25，连续两次全量通过；单次调用约 0.3–1 s。

修正项：括号/引号内的逗号不再切分；角色新增「前一项的组成部分」（里面有/包括/能/还要支持…），逐行或列表条目默认独立；截止日取同一事项各片段中最确定的回答且要求日期用途为截止；关系题接受“标题＋下方列表/冒号分解”这种结构依据；原文 `关联「X」` 精确命中已有目标时直接作为确定上级；单项预览不再显示“整体形式不确定”。

仍属 T01 范围：25 条是调优集，不是 60 条独立标注集；TypeSafe 原生与 Gateway 未用真实 Key 复测。

**第二轮（同日）：** 新增候选发现规则——看板上非 Later 的未完成目标，若与原文共享专有词（拉丁词 ≥4 字母或中文连续 ≥4 字，如 “Bottega”），即使未写全标题也进入候选（仍受 8 个上限，排在点名/用户选择之后、流程根之前），并在 state 中标出共同词；关系题接受“同一项目/产品里的工作”作为依据。说明改为截取标题之后的原文原句（保留标点，不再用换行拼接片段）。Jev 判定的上级若与新关联的时间列规则冲突（如 Later 事项的本月上级），不直接关联，而是显示为一键建议「可能的上级：X · 关联后移到本周」，点击才关联并把事项移到上级之下最长的合法列。样例扩充为 50 条（含 24 条模拟真实看板的 TODO），连续三次全量 50/50。

**第三轮（同日）：执行时间推测（方案 C，负责人选定）。** 每个槽位同轮增加一道 Choice「若原文没写执行时间，按工作量、紧迫度与所属目标所在列推测范围」（day/week/month/cycle/later）。原文写明且确定的执行时间始终优先；只有原文未写时才使用推测：确定的推测直接预填并标「推测」，可随时修改；不确定的只作建议 chip；推测为 later 不显示。推测不会越过“上级更长”的关联规则，接受上级建议时仍按该规则移列。50 条回归不变（50/50），推测结果逐条见 `output/eval` 报告的 inferred 列，由负责人判断是否合乎习惯。

### 14.5 新建界面：输入法式（2026-09-25，负责人选定，替代同日的搜索面板式）

输入框下只有一条候选：带「✦ Jev」标记的最终推荐（单件是一句话，如「放到本周，归到「X」下」；多件每行「标题 · 列」，本批上级用缩进表示），右侧主按钮文案汇总结果（保存到 Later / 创建到本周 / 创建 N 项）。键位只有三个：**↵ 执行主操作**（⇧↵ 换行，不在界面提示）、**Tab 调整**、**⌥↵ 原样存到 Later**（整段原文存为 1 条 Later，不拆分、不设时间与上级；有手动调整时先确认）。清空草稿是输入框右侧的图标按钮。

Jev 拿不准的执行列与上级默认直接采用进推荐（负责人 2026-09-25 决定）：需要时按新关联规则把事项移到上级之下最长的合法列；已手动改过该字段、或移列会破坏本批下级的关联时，保留为建议。拿不准的截止（可能是日期范围）与片段角色（是否并入上一件）不自动采用，以灰字「可能……」附在对应事项后。采用的值与普通判断一样带 ✦，可在调整菜单里改回。

Tab 进入逐项调整：每件事一行，选中行在行内露出 **T 列 / D 截止 / P 上级** 三个标签（Jev 给出的值带 ✦，拿不准的标签描蓝边），按字母弹出小菜单——列 1–5 直选、截止快捷日期或自选日期、上级搜索（Jev 建议排在最前并标 Jev，需要时注明“会移到本周”），无上级时设为新流程的颜色在 P 菜单底部；E 就地改标题与说明，⌫ 移除，M 把拿不准的片段并入上一件；↑↓ 换事项，Esc 回到输入框。整理新文本期间保留上一版推荐（变淡）而不闪烁，仍可调整，但要等整理完成才能创建。未启用 Jev 时只有「连接 Jev 自动整理 →」和「保存到 Later ↵」。设计稿见 canvas「Composer 新增弹窗重设计」的“输入法式”与“逐项调整 · 新方案 1 行内标签”画板。
