...
首次流程为：**确认既有工作区日历 → 可选启用 Jev → 看板**。Jev 步骤首先只展示轻量介绍与两个同样可见的按钮：「连接 Jev」「暂时跳过」；仅选择连接才展开服务和 Key 表单。不强制注册、跳出购买或填写 Key，不改变日历确认及锁定语义。
...
成功后可选择体验同一个 composer。示例只在点击后填入，确认前不生成任务。已有用户升级时默认关闭，从「设置 → 智能输入」启用；普通 composer 可以显示一次可关闭的设置入口，但不拦截录入，不反复要求配置。
...
Key/服务更换取消旧请求、保留手动草稿并使建议过期。跨服务切换后已有输入需用户明确重新发送，不悄悄转发给新处理方；尚未启用时输入的普通草稿同理。后续新输入按已同意的配置自动判断，不增加每次都必点的“识别”按钮。
...
关闭未保存内容可保留当前会话草稿或放弃；退出提醒未保存，整库替换清理旧草稿。保存失败不清输入、不关闭窗口。模型的选项分布不足以形成明确建议时，展示选择或保留普通输入；确定性校验通过仍必须由用户确认。
...
常规单任务、已知片段/候选只请求一次。仅当第一轮确定新分组、确实需要新状态才能定义后续问题时，可追加一次具名补充请求；记录原因并继续受累计64题约束。预算耗尽不为“问完所有关系”自动拆更多请求，改为手动确认。每轮实际问题清单由调度器先计算、校验再发送。
...
默认无Retry-After的限流冷却为30秒（应用参数，不是供应商配额）。403带账户验证类型优先归账户验证；429带明确额度类型优先归额度；未知4xx提示未能调用并给出脱敏状态，不擅自断言Key失效。只打开预设官方控制台链接，不跟随错误body里的任意URL。无跨服务自动容灾。相同有效修订缓存仅限当前会话，包含上下文/问题/模型/服务配置；重试识别不改变数据库命令幂等ID。
...
- **数据版本 v3：** 首次用新版本打开 v1/v2 工作区时，会先在原备份目录创建并校验 `protective` 副本再原子升级。**不要用旧版本直接打开已升级的 v3 工作区**（旧版会报不支持的版本）；需要回到旧数据时回到新版本，或由负责人从升级前保护副本恢复。保护或升级失败时应用不开放写入，并显示副本与数据位置。
