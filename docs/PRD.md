...
| 项目 | 内容 |
| --- | --- |
| 版本 | v0.3.1 · Electron 与历史 / 顺延方向已确认；未说明的工程细节为实施建议 |
| 日期 | 2026-09-21 |
| 产品 / 域名 | Goalloom / goalloom.com，域名购买由用户告知 |
| 平台 | macOS、Windows 本地桌面 |
| UI 约束 | shadcn/ui、Tailwind CSS、Hugeicons 图标 |
...
已确认：五列顺序、目标关联、shadcn/ui + Tailwind CSS、Hugeicons 图标、Mac / Windows、本地桌面、自定义三个月周期、单一主位置、私人仓库及 Markdown TODO。
...
任何时间列只要存在有效的往期未完成事项，就在当前模式顶部显示可折叠的「往期未完成 · N」，不以手动 / 自动策略作为显示条件。撤销后抑制、自动失败、设置生效前积压和恢复暂停的事项同样可见；已成功顺延的条目改为显示来源。当前待办和往期数量分别表达，不将旧承诺伪装成新计划。
...
应用 UI 图标统一采用用户指定的 **Hugeicons**，覆盖看板工具栏、历史翻页、关联、状态、设置、空状态和 shadcn 组件内部图标。首版默认采用免费 Stroke Rounded；具体图标与样式细节按 [图标规范](ICONOGRAPHY.md) 在 M1 实现，不默认采购 Pro。

图标随 Electron 安装包离线可用，不加载运行时 CDN、远程 SVG 或图标字体。不得为了沿用组件示例而混入 Lucide 等第二套 UI 图标；图标按钮要有可访问名称。品牌 Logo / 应用启动图标另行设计，系统原生窗口控件保留平台行为。
