...
v0.6.2 · 2026-09-22。产品规则见 [PRD](PRD.md)，执行与文档协议见 [AGENTS](../AGENTS.md)。
...
**关系双入口：** `linkItems(parentId, childId, expectedVersions, operationId, workspaceGeneration)` 服务「选择上级」和「关联已有下级」。双方须存在 / 未删除，允许任一端点已归档，选择器标注且不自动解除归档；事务校验防环 / 活跃边去重。关联 / 解除 / 换父回执保留本次边ID、端点、有效性和失效来源的前后差量；端点当前版本仅保护本次提交，不用于否决以后与该边无关的文字编辑。单父已有其他父时先确认，在同事务检查实际父集合后替换；两入口不改placement / status，取消与重试不增加边。
...
| 关联 / 解除 / 换父 | 本次边的ID、端点、有效性、失效来源及单父时的相关父集合；端点未删除，归档允许 | 只逆转边差量，重检防环 / 基数；未涉及边保留，独立重新建立的新边不能仅凭同端点冒充旧边 |
...
- [ ] 按PRD表实现全部用户撤销范围及§2.4效果契约；原创建 / 排序无Toast，所有撤销成功有信息Toast、撤销创建附定向还原；系统批次不入用户栈，确定冲突仅弹当前项。
- [ ] 测断网闭环、正文编辑不阻断移动 / 状态撤销、创建→补说明→撤销→还原内容保留、连续逆转、非栈顶Toast、真实效果 / 依赖冲突与乐观锁竞争；旧基线更新机制不保留。
