...
**当前进度（2026-09-23）：** M0 已获负责人确认（多父 DAG、固定三日历月、macOS 14+ Apple Silicon / Windows 11 x64、中文、本人内测）；M1 本机持久化闭环已实现，M2 五列/生命周期/效果撤销和 M3 历史/往期入口已通过本机真实窗口验证；正在推进 M4 顺延、备份与整库操作。跨平台安装和人工实机验收仍单独追踪，实际命令、版本和测试结果随开发提交记录。
...
- [ ] 建无Electron依赖的domain及注入时钟，将日历 / 候选 / 撤销 / 导入样例做成fixture，并单独跑repository测试。已覆盖日/周/月/原锚点三个月、候选/hold、完整用户效果撤销与独立 repository 测试；导入样例随 M4 实现。
...
- [ ] 实现聚焦列新建 / 历史回落、多行输入快捷键 / IME、未保存退出提示、中文搜索、主题 / 窄窗口；当前列无移出引用。已实现快捷键、草稿保护、搜索、持久主题和窄窗；中英文 IME 组合输入仍需 macOS / Windows 实机人工回归，不以合成事件代替。
...
- [ ] 批次按位置效果与依赖检查，保留无关文字 / 状态；所有返回旧todo的撤销均落hold，包括manual策略；重启 / 下周期 / 手动移位按契约处理，部分跳过正确反馈。
- [ ] 日常备份 / N轮换、JSON / 备份恢复与resetWorkspace的预览 / 保护备份 / 确认 / 原子替换可用；重置从备份准备起持续维护，勾选确认后才提交；恢复设暂停，重置回配置，旧代次写入 / Toast失效。
