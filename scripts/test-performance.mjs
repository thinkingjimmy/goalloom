...
await build({ configFile: false, build: { outDir: '.electron-test', emptyOutDir: false, lib: { entry: 'tests/fixtures/performance.ts', formats: ['cjs'], fileName: () => 'performance.cjs' }, rollupOptions: { external: [/^node:/] }, minify: false } })
const profile = await mkdtemp(join(tmpdir(), 'Goalloom 性能验收 '))
let app
try {
  await new Promise((resolve, reject) => {
    const child = spawn(electronPath, ['.electron-test/performance.cjs', profile], { env: { ...process.env, ELECTRON_RUN_AS_NODE: '1' }, stdio: 'inherit' })
  ...
  await page.waitForFunction(() => document.querySelectorAll('.task-card').length === 1000)
  ...
  await page.getByRole('searchbox', { name: '搜索标题或说明' }).fill('中文')
  await page.getByText('10000 个条目', { exact: true }).waitFor()
  ...
  assert.equal(await page.locator('.list-item').count(), 50)
  await mkdir('output/performance', { recursive: true })
  await writeFile('output/performance/latest.json', JSON.stringify(report, null, 2))
