/**
 * [INPUT]: Private builder output, platform language manifests and measured size budgets.
...
const archive = (await readdir(root)).find(name => platform === 'darwin-arm64' ? name.endsWith('.zip') : name.endsWith('.exe'))
assert(archive, 'Download artifact missing')
